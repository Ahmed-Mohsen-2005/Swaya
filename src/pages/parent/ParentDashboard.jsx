...
import { displayName, initialsForEntity, shortDisplayName } from '../../utils/localization';
...
export default function ParentDashboard() {
  ...
  const childName = displayName(child, language);
  const childShortName = shortDisplayName(child, language);
  const state = live.status === 'active' ? generalState(metrics) : 'Calm';

  return (
    <div className="grid">
      <PageHeader title="Parent overview" subtitle={`${childShortName} ${t('is being monitored with supportive classroom insights.')}`} meta={{label:'Last updated', value:relativeTime(loadedAt)}}/>
      <div className="grid parent-overview-grid">
        <Card className="child-profile-card">
          <div className="priority-person"><div className="avatar avatar-lg">{initialsForEntity(child, language)}</div><div><h2>{childName}</h2><p>{t('Inclusive Class A')}</p></div></div>
      ...
      <div className="grid parent-dashboard-content-grid">
        <ChartCard title={{ label: "{{name}}'s Progress", vars: { name: childShortName } }} subtitle="Friendly trend view for today" actions={<><button className="pill active">{t('Today')}</button><Link className="analytics-link" to="/parent/progress">{t('View progress')}</Link></>}><LiveLineChart data={trend}/></ChartCard>
      ...
      <div className="grid grid-3">
        <Card><h2 style={{ marginTop: 0 }}>{t('Home Recommendations')}</h2>{recommendations.map(r => <div className="student-card" key={r.id}><b>{t(r.title)}</b><p className="muted">{t(r.simplifiedDescription)}</p></div>)}<Link to="/parent/recommendations"><Button variant="outline" style={{ width: '100%' }}>{t('View Recommendations')}</Button></Link></Card>
        <Card><h2 style={{ marginTop: 0 }}>{t('Teacher Note')}</h2><p>{childShortName} {t('responded well after calm mode was activated and participated more actively.')}</p><p className="small muted">{t('Mona Hassan')}, {t('Teacher')}</p></Card>
        <Card><h2 style={{ marginTop: 0 }}>{t('Upcoming Session')}</h2><p><Calendar size={16}/> {t('Tomorrow, May 27')}<br/>09:00 - 09:40<br/>{t('Inclusive Class A')}</p></Card>
      </div>
    </div>
  );
}
