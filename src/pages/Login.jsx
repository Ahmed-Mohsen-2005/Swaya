...
const roleConfig = {
  teacher: { email: 'teacher@swaya.demo', icon: Users, label: 'Teacher', description: 'Live classroom monitoring and student support workflows.' },
  doctor: { email: 'doctor@swaya.demo', icon: Stethoscope, label: 'Doctor', description: 'Clinical review, recommendations, and therapy planning.' },
  parent: { email: 'parent@swaya.demo', icon: UserRound, label: 'Parent', description: 'Parent-safe progress updates and home guidance.' },
};
...
export default function Login() {
  ...
  return (
    <div className="auth-shell">
      <button className="auth-language" onClick={toggleLanguage}><Languages size={16}/>{isArabic ? 'English' : 'العربية'}</button>
      ...
      <section className="auth-panel-wrap">
        <form className="auth-panel" onSubmit={submit}>
          ...
          <div className="auth-form">
            <label>
              ...
              <div className="auth-input">
                <Mail size={17}/>
                <input value={email} onChange={event => setEmail(event.target.value)} autoComplete="email" placeholder={activeRole.email}/>
              </div>
            </label>
            <label>
              ...
              <div className="auth-input">
                <LockKeyhole size={17}/>
                <input type={showPassword ? 'text' : 'password'} value={password} onChange={event => setPassword(event.target.value)} autoComplete="current-password" placeholder={t('Enter your password')}/>
                <button type="button" className="password-toggle" onClick={() => setShowPassword(value => !value)} aria-label={t(showPassword ? 'Hide password' : 'Show password')}>{showPassword ? <EyeOff size={16}/> : <Eye size={16}/>}</button>
              </div>
            </label>
            <div className="auth-form-row">
              <label className="remember-control"><input type="checkbox" checked={remember} onChange={event => setRemember(event.target.checked)}/><span>{t('Remember me')}</span></label>
              <button type="button" className="forgot-link" onClick={() => setNote('Password reset is not connected in this demo environment.')}>{t('Forgot password?')}</button>
            </div>
            {err && <div className="auth-alert error">{t(err)}</div>}
            {note && <div className="auth-alert info">{t(note)}</div>}
            <Button type="submit" style={{ width: '100%' }} loading={loading}>{t('Enter Workspace')}</Button>
          </div>
          ...
          <div className="auth-trust">
            {['Secure access', 'Role-based permissions', 'Encrypted data flow', 'Arabic/English support'].map(item => <span key={item}><ShieldCheck size={13}/>{t(item)}</span>)}
          </div>
        </form>
      </section>
    </div>
